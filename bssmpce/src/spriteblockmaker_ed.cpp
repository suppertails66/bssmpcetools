...
/*  for (int i = 0; i < 16; i++) {
  ...
  vector<OutputPattern> outputPatterns2;
  convertToPatterns(grp2, outputPatterns2);
...
//  int numHorizontalPatterns2 = (grp2.w() / PceSpritePattern::w);
//  if ((grp2.w() % PceSpritePattern::w) != 0) ++numHorizontalPatterns2;
  int numVerticalPatterns2 = (grp2.h() / PceSpritePattern::h);
  if ((grp2.h() % PceSpritePattern::h) != 0) ++numVerticalPatterns2;
...
//  int graphic1OffsetY = (targetH - grp1.h()) / 2;
  ...
  int graphic2OffsetX = (targetW - grp2.w()) / 2;
//  int graphic2OffsetY = (targetH - grp2.h()) / 2;
  int graphic2OffsetY
    = (targetH - (numVerticalPatterns2 * PceSpritePattern::h)) / 2;
...
//  ofs.writeu8(outputPatterns2.size());
  for (int i = 0; i < outputPatterns2.size(); i++) {
    OutputPattern& pattern = outputPatterns2[i];
    pattern.pattern.write(ofs);
  }
...
//  ofs.writeu16le(outputPatterns2.size());
  for (int i = 0; i < outputPatterns2.size(); i++) {
    OutputPattern& pattern = outputPatterns2[i];
    
    int x = 32 + graphic2OffsetX + pattern.x;
    int y = 64 + graphic2OffsetY + pattern.y + spriteBaseY2;
