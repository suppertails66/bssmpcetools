...
//  for (std::map<std::string, bool>::iterator it = labels.begin();
//       it != labels.end();
//       ++it) {
//    std::cout << it->first << "," << std::endl;
//  }
...
//  for (std::map<std::string, bool>::iterator it = unusedHashes.begin();
//       it != unusedHashes.end();
//       ++it) {
//    std::cout << it->first << std::endl;
//    
//    SmPceFileIndexEntry* indexEntry = index.findEntryByHash(it->first);
//    TBufStream scrifs(indexEntry->getByteSize());
//    ifs.seek(indexEntry->getBytePos());
//    scrifs.writeFrom(ifs, indexEntry->getByteSize());
//    scrifs.seek(0);
//    scrifs.save((std::string("unusedfiles/") + it->first).c_str());
//  }


//  TIfstream ifs("1D014B.bin", ios_base::binary);
